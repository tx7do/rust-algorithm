...
/// quick_sort with Lomuto parition scheme.
...
/// Tail-call opitimized quick_sort with Lomuto parition scheme.
pub fn quick_sort_optimized(arr: &mut [i32]) {
    let hi = arr.len() as isize - 1;
    quick_sort_helper_optimized(arr, 0, hi);
}
...
fn quick_sort_helper_optimized(arr: &mut [i32], lo: isize, hi: isize) {
    if lo <= hi {
        let pivot = partition(arr, lo, hi);
        if pivot - lo < hi - pivot {
            // 1
            quick_sort_helper_optimized(arr, lo, pivot - 1);
            quick_sort_helper_optimized(arr, pivot + 1, hi);
        } else {
            quick_sort_helper_optimized(arr, pivot + 1, hi);
            quick_sort_helper_optimized(arr, lo, pivot - 1);
        }
...
/// Manual tail-call opitimized quick_sort with Lomuto parition scheme.
pub fn quick_sort_manual_tco(arr: &mut [i32]) {
    let hi = arr.len() as isize - 1;
    quick_sort_helper_manual_tco(arr, 0, hi);
}
...
fn quick_sort_helper_manual_tco(arr: &mut [i32], mut lo: isize, mut hi: isize) {
    while lo < hi {
        let pivot = partition(arr, lo, hi);
        if pivot - lo < hi - pivot {
            quick_sort_helper_manual_tco(arr, lo, pivot - 1);
            lo = pivot + 1;
        } else {
            quick_sort_helper_manual_tco(arr, pivot + 1, hi);
            hi = pivot - 1;
        }
    }
}
...
/// quick_sort with 3-way parition scheme.
pub fn quick_sort_3way(arr: &mut [i32]) {
    let hi = arr.len() as isize - 1;
    quick_sort_helper_3way(arr, 0, hi);
}

/// Recursion helper
fn quick_sort_helper_3way(arr: &mut [i32], lo: isize, hi: isize) {
    if lo <= hi {
        let (smaller, larger) = partition_3way(arr, lo, hi);
        quick_sort_helper_3way(arr, lo, smaller - 1);
        quick_sort_helper_3way(arr, larger + 1, hi);
    }
}
...
/// quick_sort with Hoare parition scheme
pub fn quick_sort_hoare(arr: &mut [i32]) {
    if arr.is_empty() {
        return;
    }
    let hi = arr.len() - 1;
    quick_sort_helper_hoare(arr, 0, hi);
}

/// Recursion helper
fn quick_sort_helper_hoare(arr: &mut [i32], lo: usize, hi: usize) {
    if lo < hi {
        let pivot = partition_hoare(arr, lo, hi);
        quick_sort_helper_hoare(arr, lo, pivot);
        quick_sort_helper_hoare(arr, pivot + 1, hi);
    }
}
...
mod base {
    use super::*;
    base_cases!(quick_sort);
}

#[cfg(test)]
mod optimized {
    use super::*;
    base_cases!(quick_sort_optimized);
}

#[cfg(test)]
mod manual_tco {
    use super::*;
    base_cases!(quick_sort_manual_tco);
}

#[cfg(test)]
mod three_way {
    use super::*;
    base_cases!(quick_sort_3way);
}

#[cfg(test)]
mod hoare {
    use super::*;
    base_cases!(quick_sort_hoare);
}
